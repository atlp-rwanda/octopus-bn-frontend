...
import { render, waitFor, cleanup, fireEvent, getAllByAltText } from '@testing-library/react';
...
describe("TESTING RESET EMAIL SENDING ACTIVITY", () => {
  ...
  it('should test the sendMailAction', async (done, data) => {
    try {
      ...
      expect(mock).toHaveBeenCalledWith('https://octopus-bn-backend.herokuapp.com/api/v1/auth/forgot-password');
    } catch (err) {
      if (err.response) {
  ...
  it('should set an email value', (done) => {
    ...
    expect(dispatch.mock.calls[1][0].type).toEqual(types.SEND_EMAIL_SUCCESS);
    done()
  })
