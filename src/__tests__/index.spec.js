...
describe('SIGN UP', () => {
	...
	it('should show an error when there is no firstname provided', () => {
		const { getByLabelText, getByText, getByTestId, container, debug } = signupComponent();
		const firstName = getByLabelText('First Name');
		const lastName = getByLabelText('Last Name');
		const email = getByLabelText('email');
		const password = getByLabelText('password');
		...
		waitFor(() => expect(getByText('Your first name is required')).toBeTruthy());
	});
	...
	it('should show an error when there is no password provided', () => {
		...
		form.dispatchEvent(new Event('submit'));
		waitFor(() => expect(getByText('minimum lenght for password is 8 characters')).toBeTruthy());
	});
